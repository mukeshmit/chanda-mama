        ...
        <div class="page-heading">
            <div class="page-title mb-2">
                <div class="d-flex justify-content-between align-items-center">
                    <h3 class="modern-page-title mb-0">Sub Sub Sub Category</h3>
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item"><router-link to="/dashboard" class="text-muted">{{
                                    __('dashboard') }}</router-link></li>
                            <li class="breadcrumb-item active text-primary" aria-current="page">Sub Sub Sub Category</li>
                        </ol>
                    </nav>
                </div>
            </div>
            ...
            <section class="section">
                <div class="figma-main-section-card">
                    <div class="card-body p-0">
                        ...
                        <div v-else-if="!hasSubSubcategories" class="alert alert-warning m-3">
                            <i class="fa fa-exclamation-triangle"></i>
                            {{ __('no_sub_subcategories_found') }}
                            <router-link to="/manage_sub_subcategories/create" class="btn btn-primary btn-sm ml-2">
                                {{ __('create_sub_subcategory_first') }}
                            </router-link>
                        </div>
                        ...
                        <div
                            ...
                            <div class="d-flex gap-2 align-items-center flex-wrap">
                                ...
                                <button class="btn btn-figma-columns d-flex align-items-center gap-2"
                                    @click="create_new = true" v-if="$can('category_create') && hasParentCategories && hasSubcategories && hasSubSubcategories">
                                    <i class="fa fa-plus"></i>
                                    <span>Add Sub Sub Sub Category</span>
                                </button>
                            </div>
                        </div>
...
import EditRecord from './EditSubSubSubcategory.vue';
export default {
    components: {
    ...
    data: function () {
        return {
            fields: [
                ...
                { key: 'parent_sub_subcategory', label: this.$titleLabel('parent_sub_subcategory'), class: 'text-center' },
    ...
    methods: {
        ...
        getSubSubcategories() {
            return axios.get(this.$apiUrl + '/categories', {
                ...
                .then((response) => {
                    ...
                    this.subSubcategories = allCategories.filter(cat => subcategoryIds.includes(Number(cat.parent_id)));
                    this.hasSubSubcategories = this.subSubcategories.length > 0;
                })
                .catch(() => {
        ...
        deleteCategory(index, id) {
            this.$swal.fire({
                ...
                if (result.value) {
                    ...
                    axios.post(this.$apiUrl + '/sub_sub_subcategories/delete', postData)
                        .then((response) => {
                            this.isLoading = false
        ...
        hideModal() {
            ...
            this.$router.push({ path: '/manage_sub_sub_subcategories' });
        },
