                ...
                <form id="payment_method_settings_form" method="post" enctype="multipart/form-data" @submit.prevent="saveRecord">
                    <div class="row">
                        ...
                        <div class="col-6 mb-4">
                            <div class="card h-100">
                                <div class="card-header">
                                    <h4 class="card-title">{{ __('dpo_payments') }}</h4>
                                </div>
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="dpo_payment_method">{{ __('dpo_payments') }}<small>[ {{ __('enable') }} / {{ __('disable') }} ] </small></label><br>
                                        <div class='form-check form-switch'>
                                            <input class='form-check-input' id="dpo_payment_method" type='checkbox' true-value="1" false-value="0" v-model="payment_method.dpo_payment_method" :checked="payment_method.dpo_payment_method">
                                        </div>
                                    </div>
                                    <div v-if="payment_method.dpo_payment_method == 1" class="form-group">
                                        <label for="dpo_mode">{{ __('dpo_payment_mode') }} <small>[ {{ __('sandbox') }} / {{ __('live') }} ]</small></label>
                                        <select name="dpo_mode" id="dpo_mode" :required="payment_method.dpo_payment_method == 1" v-model="payment_method.dpo_mode" class="form-control form-select">
                                            <option value="">{{ __('select_mode') }}</option>
                                            <option value="sandbox">{{ __('sandbox') }} (  {{ __('testing') }})</option>
                                            <option value="production">{{ __('production') }} (  {{ __('live') }})</option>
                                        </select>
                                    ...
                                    <div v-if="payment_method.dpo_payment_method == 1" class="form-group">
                                        <label for="dpo_company_token">{{ __('dpo_company_token') }}</label>
                                        <input type="text" class="form-control" name="dpo_company_token" id="dpo_company_token" v-model="payment_method.dpo_company_token" placeholder="DPO Company Token">
                                    </div>
                                    <div v-if="payment_method.dpo_payment_method == 1" class="form-group">
                                        <label for="dpo_service_type">{{ __('dpo_service_type') }}</label>
                                        <input type="text" class="form-control" name="dpo_service_type" id="dpo_service_type" v-model="payment_method.dpo_service_type" placeholder="DPO Service Type (e.g. 3854)">
                                    </div>
                                    <div v-if="payment_method.dpo_payment_method == 1" class="form-group">
                                        <label for="dpo_currency_code">{{ __('currency_code') }} <small>[ {{ __('dpo_supported') }} ]</small> <a href="https://docs.directpay.online/" target="_BLANK"><i class="fa fa-link"></i></a></label>
                                        <select name="dpo_currency_code" id="dpo_currency_code" :required="payment_method.dpo_payment_method == 1" v-model="payment_method.dpo_currency_code" class="form-control form-select">
                                            <option value="">{{ __('select_currency_code') }}</option>
                                    ...
                                    <div v-if="payment_method.dpo_payment_method == 1" class="form-group">
                                        <label for="dpo_notification_url">{{ __('notification_url') }} <small>{{ __('set_webhook_url_for_dpo') }}</small></label>
                                        <input type="text" class="form-control" name="dpo_notification_url" v-model="payment_method.dpo_notification_url" placeholder="DPO Webhook URL" disabled="">
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        getPaymentMethods() {
            axios.get(this.$apiUrl + '/payment_methods').then((response) => {
                if(response.data.data) {
                    ...
                    this.payment_method.dpo_payment_method = this.record.dpo_payment_method;
                    this.payment_method.dpo_mode = this.record.dpo_mode;
                    this.payment_method.dpo_company_token = this.record.dpo_company_token;
                    this.payment_method.dpo_service_type = this.record.dpo_service_type;
                    this.payment_method.dpo_currency_code = this.record.dpo_currency_code ?? "";
                }
            });
        },
        saveRecord: function(){
            ...
            if (this.payment_method.cod_payment_method == 1 ||
                ...
                this.payment_method.dpo_payment_method == 1) {
