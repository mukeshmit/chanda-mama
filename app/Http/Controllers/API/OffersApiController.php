...
class OffersApiController extends Controller
{
    public function index()
    {
        ...
        $offers = Offer::with([
            'category' => function ($q) {
                $q->where('status', '!=', 0)->with('translations');
            },
            ...
            ->get()
            ->map(function ($offer) {
                // Convert offer to array to ensure all data is included
                $offerArray = $offer->toArray();

                // Ensure translations are included as arrays (not accessors)
                if ($offer->category) {
                    // Get category as array
                    $categoryArray = $offer->category->toArray();
                    // Hide the translations accessor that returns only current language
                    $offer->category->makeHidden(['translations']);
                    // Add full translations array from relation
                    if ($offer->category->relationLoaded('translations')) {
                        $categoryArray['translations'] = $offer->category->getRelation('translations')->toArray();
                    } else {
                        $categoryArray['translations'] = [];
                    }
                    // Update the category in offer array
                    $offerArray['category'] = $categoryArray;
                }

                if ($offer->product) {
                    // Get product as array
                    $productArray = $offer->product->toArray();
                    // Hide the translations accessor that returns only current language
                    $offer->product->makeHidden(['translations']);
                    // Add full translations array from relation
                    if ($offer->product->relationLoaded('translations')) {
                        $productArray['translations'] = $offer->product->getRelation('translations')->toArray();
                    } else {
                        $productArray['translations'] = [];
                    }
                    // Update the product in offer array
                    $offerArray['product'] = $productArray;
                }
    ...
    public function update(Request $request)
    {
        ...
        return CommonHelper::responseSuccess('offer_updated_successfully');
    }
    public function delete(Request $request)
    {
        if (isset($request->id)) {
            ...
            if ($offer) {
                ...
                return CommonHelper::responseSuccess('offer_already_deleted');
            }
        }
    }
